...
function PlacementAdditionModal(props) {
    ...
    const PostData =async (e)=>{
            if(start===''||end===''){
              ...
              alert('Please upload file');
            ...
            else if(window.dataParse[0][0]!=="SNO" || window.dataParse[0][1]!=="ROLLNO" || window.dataParse[0][2]!=="NAME"||window.dataParse[0][3]!=="COMPANY NAME"||window.dataParse[0][4]!=="Salary Package"){
                ...
                for(var i=1;i<window.dataParse.length;i++){
                    const res=fetch("/api/placements",{
                        method:"POST",
                        headers:{
                          "Content-type":"application/json"
                        },
                        body:JSON.stringify({
                          "sNo":window.dataParse[i][0],"rollNo":window.dataParse[i][1],"name":window.dataParse[i][2],"company":window.dataParse[i][3],"salary":window.dataParse[i][4],"batch":start+"-"+end
                        })
                    });
                }
      ...
      >
        ...
        <Modal.Body>
          <form method="POST"> 
              ...
              <label className='col-sm-8' >Upload file<b>[only upload files with xlsx format.Headers in the file should be <i>"SNO"</i><span>,</span><i>"ROLLNO"</i><span>,</span><i>"NAME"</i><span>,</span><i>"COMPANY NAME"</i><span>,</span><i>"Salary Package"</i>.Otherwise the data in the file will not be added]</b></label>
  ...
  export default PlacementAdditionModal;