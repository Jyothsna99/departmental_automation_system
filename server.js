...
app.get("/api/seminar/:from/:to",async (req,res)=>{
    let fromP=req.params.from;
    let toP=req.params.to;
    let fromQuery="";
    let toQuery="";
    ...
    console.log(fromQuery+toQuery);
    const award=await Seminar.find({
        "date":JSON.parse(fromQuery+toQuery)
    });
    console.log(award);
    res.send(award);
...
app.put("/api/seminar/:id",async(req,res)=>{
    const updatedCoordinator=await Seminar.updateOne({ _id: req.params.id },{ $set: {topic:req.body.topic, resourcePerson:req.body.resourcePerson,venue:req.body.venue, date:req.body.date,participants:req.body.participants,who:req.body.who} });
...
app.get("/api/fdpOrganized/:from/:to",async (req,res)=>{
    let fromP=req.params.from;
    let toP=req.params.to;
    let fromQuery="";
    let toQuery="";
    ...
    console.log(fromQuery+toQuery);
    const award=await FDPOrganized.find({
        "date":JSON.parse(fromQuery+toQuery)
    });
    console.log(award);
    res.send(award);
...
app.put("/api/fdpOrganized/:id",async(req,res)=>{
    const updatedCoordinator=await FDPOrganized.updateOne({ _id: req.params.id },{ $set: {name:req.body.name, org:req.body.org, venue:req.body.venue,resourcePerson:req.body.resourcePerson, date:req.body.date,nop:req.body.nop,stuFac:req.body.stuFac} });
    res.send(updatedCoordinator);
})
...
app.get("/api/workshopAttended/:name/:type/:from/:to",async (req,res)=>{
    
    let nameP=req.params.name;
    let typeP=req.params.type;
    let fromP=req.params.from;
    let toP=req.params.to;
    let dateQuery="";
    let nameQuery="";
    ...
    dateQuery=fromQuery+toQuery;
    console.log(dateQuery);
    if(dateQuery!=="{}"){
        dateQuery=JSON.parse("{\"from\":"+ dateQuery+"}");
    }else{
        dateQuery=JSON.parse("{}");
    }
    
    console.log(dateQuery);
    if(nameP!=="null" && nameP!=="Select Name of the Program"){
        nameQuery="{\"name\":\""+ nameP+"\"}";
        nameQuery=JSON.parse(nameQuery);
    }else{
        nameQuery=JSON.parse("{}");
    }
    if(typeP!=="null" &&typeP!=="Select Type"){
        typeQuery="{\"type\":\""+ typeP+"\"}";
        typeQuery=JSON.parse(typeQuery);
        
    }else{
        typeQuery=JSON.parse("{}");
    }
    console.log(nameQuery,typeQuery,dateQuery);
    const syllabus=await WorkshopAttended.find({
        $and:[
            dateQuery,
            nameQuery,
            typeQuery,
...
const StudentActivities=mongoose.model("studentActivities",new mongoose.Schema({
    _id:{type:String, default:shortid.generate},
    eventName:{
        type:String,
...
app.get("/api/studentActivities",async (req,res)=>{
    const award=await StudentActivities.find({});
    res.send(award);
});

app.delete("/api/studentActivities/:id",async(req,res)=>{
    const deletedAward=await StudentActivities.deleteOne({_id:req.params.id});
    res.send(deletedAward);
});

app.post("/api/studentActivities",async(req,res) => {
    const newSubjectAllocation = new StudentActivities(req.body);
    await newSubjectAllocation.save()
    .then((response) =>{
        console.log(response);
        res.send(response);
       ...
       .catch( (error)=> {
        let err="";
        if(error.errors!==undefined){
            if(error.errors.eventName!==undefined){
                err=error.errors.eventName.properties.message;
            }
            else if(error.errors.studentName!==undefined){
                err=error.errors.studentName.properties.message;
            }
        } 
        return res.status(400).json({
            "error": err
...
app.get("/api/studentActivities/:from/:to",async (req,res)=>{
    let fromP=req.params.from;
    let toP=req.params.to;
    let fromQuery="";
    let toQuery="";
    ...
    console.log(fromQuery+toQuery);
    const studentActivities=await StudentActivities.find({
        "from":JSON.parse(fromQuery+toQuery)
    });
    res.send(studentActivities);
});

app.put("/api/studentActivities/:id",async(req,res)=>{
    console.log("student Activities",req);
    const updatedAward=await StudentActivities.updateOne({ _id:req.params.id },{ $set: {eventName:req.body.eventName, studentName:req.body.studentName, prize:req.body.prize, nameOfProgram:req.body.nameOfProgram, organizedBy:req.body.organizedBy, from:req.body.from, to:req.body.to} });
    res.send(updatedAward);
})

/*Student Coordinator*/
const StudentCertifications=mongoose.model("studentCertifications",new mongoose.Schema({
    _id:{type:String, default:shortid.generate},
    sNo:String,
    courseName:String,
    name:String,
...
/** Coordinator Data */
app.get("/api/studentCertifications",async (req,res)=>{
    const coordinator=await StudentCertifications.find({});
    console.log(coordinator);
    res.send(coordinator);
});

app.post("/api/studentCertifications",async (req,res)=>{
    console.log("body",req.body);
    const newStudentCertification=new StudentCertifications(req.body);
    console.log("student",newStudentCertification);
    await newStudentCertification.save()
    .then((response) =>{
        console.log(response);
        res.send(response);
       })
...
app.delete("/api/studentCertifications/:id",async(req,res)=>{
    const deletedCoordinator=await StudentCertifications.deleteOne({_id:req.params.id});
    res.send(deletedCoordinator);
})

app.put("/api/studentCertifications/:id",async(req,res)=>{
    const updatedCoordinator=await StudentCertifications.updateOne({ _id: req.params.id },{ $set: {courseName:req.body.courseName, name:req.body.name, rollNo:req.body.rollNo, score:req.body.score,type:req.body.type,topper:req.body.topper,cycle:req.body.cycle,year:req.body.year} });
    res.send(updatedCoordinator);
})

app.get("/api/studentCertifications/:year/:cycle",async (req,res)=>{
    let yearP=req.params.year;
    let cycleP=req.params.cycle;
    console.log(yearP,cycleP);
    let yearQuery="";
    ...
    console.log(count);
    const certification=await StudentCertifications.find({
        $and:[
            yearQuery,
            cycleQuery
        ]
...
/*Placements*/
const Placement=mongoose.model("placement",new mongoose.Schema({
    _id:{type:String, default:shortid.generate},
    sNo:String,
    rollNo:String,
    name:String,
...
app.get("/api/placements",async (req,res)=>{
    const coordinator=await Placement.find({});
    console.log(coordinator);
    res.send(coordinator);
});

app.post("/api/placements",async (req,res)=>{
    const newStudentCertification=new Placement(req.body);
    await newStudentCertification.save()
    .then((response) =>{
        console.log(response);
        res.send(response);
...
app.delete("/api/placements/:id",async(req,res)=>{
    const deletedCoordinator=await Placement.deleteOne({_id:req.params.id});
    res.send(deletedCoordinator);
})

app.put("/api/placements/:id",async(req,res)=>{
    const updatedCoordinator=await Placement.updateOne({ _id: req.params.id },{ $set: {rollNo:req.body.rollNo, name:req.body.name, company:req.body.company, salary:req.body.salary,batch:req.body.batch} });
    res.send(updatedCoordinator);
})

app.get("/api/placements/:batch",async (req,res)=>{
    const coordinator=await Placement.find({batch:req.params.batch});
    res.send(coordinator);
});
...
app.get("/api/internships",async (req,res)=>{
    const coordinator=await Internship.find({});
    console.log(coordinator);
    res.send(coordinator);
});

app.post("/api/internships",async (req,res)=>{
    const newStudentCertification=new Internship(req.body);
    await newStudentCertification.save()
    .then((response) =>{
        console.log(response);
        res.send(response);
...
app.delete("/api/internships/:id",async(req,res)=>{
    const deletedCoordinator=await Internship.deleteOne({_id:req.params.id});
    res.send(deletedCoordinator);
})

app.put("/api/internships/:id",async(req,res)=>{
    const updatedCoordinator=await Internship.updateOne({ _id: req.params.id },{ $set: {rollNo:req.body.rollNo, name:req.body.name, class:req.body.class,company:req.body.company, dates:req.body.dates,batch:req.body.batch} });
    res.send(updatedCoordinator);
})

app.get("/api/internships/:batch",async (req,res)=>{
    const coordinator=await Internship.find({batch:req.params.batch});
    res.send(coordinator);
});
/**Seminars and Guest Lectures Organized */
const SemesterResult=mongoose.model("semesterResult",new mongoose.Schema({
    _id:{type:String, default:shortid.generate},
    semester:String,
    aSec:String,
    bSec:String,
...
/**Seminars and Guest Lectures Organized */
app.post("/api/semesterResult",async (req,res)=>{
    const newSeminar=new SemesterResult(req.body);
    await newSeminar.save()
    .then((response) =>{
        console.log(response);
        res.send(response);
...
app.get("/api/semesterResult/",async (req,res)=>{
    const seminar=await SemesterResult.find({});
    res.send(seminar);
});
app.delete("/api/semesterResult/:id",async(req,res)=>{
    const deletedSeminar=await SemesterResult.deleteOne({_id:req.params.id});
    res.send(deletedSeminar);
})

app.get("/api/semesterResult/:year",async (req,res)=>{
    const coordinator=await SemesterResult.find({year:req.params.year});
    res.send(coordinator);
});

app.put("/api/semesterResult/:id",async(req,res)=>{
    const updatedCoordinator=await SemesterResult.updateOne({ _id: req.params.id },{ $set: {semester:req.body.semester, aSec:req.body.aSec,bSec:req.body.bSec, cSec:req.body.cSec,overall:req.body.overall,year:req.body.year} });
    res.send(updatedCoordinator);
})
/**Seminars and Guest Lectures Organized */
const Activities=mongoose.model("activities",new mongoose.Schema({
    _id:{type:String, default:shortid.generate},
    name:String,
    date:Date,
    winner:String,
...
app.post("/api/activities",async (req,res)=>{
    ...
    const newSeminar=new Activities(req.body);
    await newSeminar.save()
    .then((response) =>{
        console.log(response);
        res.send(response);
...
app.get("/api/activities/",async (req,res)=>{
    const seminar=await Activities.find({});
    res.send(seminar);
});
app.delete("/api/activities/:id",async(req,res)=>{
    const deletedSeminar=await Activities.deleteOne({_id:req.params.id});
    res.send(deletedSeminar);
})

app.put("/api/activities/:id",async(req,res)=>{
    const updatedCoordinator=await Activities.updateOne({ _id: req.params.id },{ $set: {name:req.body.name, date:req.body.date,winner:req.body.winner, runner:req.body.runner} });
    res.send(updatedCoordinator);
})

app.get("/api/activities/:from/:to",async (req,res)=>{
    let fromP=req.params.from;
    let toP=req.params.to;
    let fromQuery="";
    let toQuery="";
    ...
    console.log(fromQuery+toQuery);
    const award=await Activities.find({
        "date":JSON.parse(fromQuery+toQuery)
    });
    console.log(award);
    res.send(award);
